...
void main() {
  late OpenDirectChatAndSaveToDbUseCase useCase;
  ...
  setUpAll(() {
    ...
    registerFallbackValue(OpenDirectChatRequest(friendAccountId: ''));
  ...
  setUp(() {
    ...
    useCase = OpenDirectChatAndSaveToDbUseCase(
  ...
  group('OpenDirectChatAndSaveToDbUseCase', () {
    test('should return room entity when successful', () async {
      // Arrange
      final request = OpenDirectChatRequest(friendAccountId: 'test-account-id');
      ...
      final response = OpenDirectChatResponse(room: roomEntity, roomSub: roomSubEntity);
      ...
      when(() => mockChatRoomServerRepository.openDirectChat(request)).thenAnswer((_) async => response);
    ...
    test('should return null when server response is null', () async {
      ...
      when(() => mockChatRoomServerRepository.openDirectChat(request)).thenAnswer((_) async => null);
    ...
    test('should handle response with room but no roomSub', () async {
      ...
      final response = OpenDirectChatResponse(room: roomEntity, roomSub: null);
      final membersResponse = PaginationPayload<RoomMemberEntity>(
        data: [],
        page: 1,
        pageSize: 10,
