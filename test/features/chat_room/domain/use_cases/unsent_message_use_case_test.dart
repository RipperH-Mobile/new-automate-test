...
class MockTrackedEventBus extends Mock implements TrackedEventBus {}

// Fake Classes for Fallback Values
class FakeUnsentMessageParams extends Fake implements UnsentMessageParams {}
...
void main() {
  late UnsentMessageUseCase useCase;
  ...
  setUpAll(() {
    ...
    registerFallbackValue(FakeUnsentMessageParams());
  ...
  group('UnsentMessageUseCase', () {
    group('constructor', () {
      test('Given valid repositories, When UnsentMessageUseCase is created, Then initializes correctly', () {
        // Given
        final serverRepo = MockMessageServerRepository();
        final localRepo = MockMessageLocalRepository();

        // When
        final result = UnsentMessageUseCase(
    ...
    group('call', () {
      test(
        ...
        when(() => mockMessageServerRepository.unsentMessage(any())).thenAnswer((_) async {});

        // When
        await useCase.call(params);

        // Then
        verify(() => mockMessageServerRepository.unsentMessage(params)).called(1);
      ...
      test('Given server repository throws exception, When call is executed, Then exception is propagated', () async {
        ...
        when(() => mockMessageServerRepository.unsentMessage(any())).thenThrow(testException);
