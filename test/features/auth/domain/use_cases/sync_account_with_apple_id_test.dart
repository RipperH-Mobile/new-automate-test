...
void main() {
  ...
  group('SyncAccountWithAppleID', () {
    group('call', () {
      test('Given valid request, When use case is called, Then returns SocialLinkEntity successfully', () async {
        // Given
        when(() => mockAuthServerRepository.settingAccountLinkAccountWithApple(any()))
            .thenAnswer((_) async => testSocialLinkEntity);

        // When
        final result = await GetIt.I<AuthServerRepository>().settingAccountLinkAccountWithApple(testRequest);

        // Then
        expect(result, equals(testSocialLinkEntity));
        expect(result.appleId, equals('test@apple.com'));
        expect(result.isSuccess, isTrue);
        verify(() => mockAuthServerRepository.settingAccountLinkAccountWithApple(testRequest)).called(1);
      ...
      test('Given successful linking, When use case is called, Then returns entity with success flag true', () async {
        ...
        // Then
        expect(result.isSuccess, isTrue);
        expect(result.appleId, equals('success@apple.com'));
        verify(() => mockAuthServerRepository.settingAccountLinkAccountWithApple(testRequest)).called(1);
      });
      ...
      test('Given repository throws Exception, When use case is called, Then rethrows the same exception', () async {
        ...
        when(() => mockAuthServerRepository.settingAccountLinkAccountWithApple(any())).thenThrow(exception);

        // When/Then
        await expectLater(
          () => GetIt.I<AuthServerRepository>().settingAccountLinkAccountWithApple(testRequest),
      ...
      test('Given empty token in request, When use case is called, Then rethrows the same exception', () async {
        ...
        // When/Then
        await expectLater(
          () => GetIt.I<AuthServerRepository>().settingAccountLinkAccountWithApple(emptyTokenRequest),
          throwsA(equals(exception)),
        );
        verify(() => mockAuthServerRepository.settingAccountLinkAccountWithApple(emptyTokenRequest)).called(1);
      });
      ...
      test('Given empty action token in request, When use case is called, Then rethrows the same exception', () async {
        ...
        // When/Then
        await expectLater(
          () => GetIt.I<AuthServerRepository>().settingAccountLinkAccountWithApple(emptyActionTokenRequest),
          throwsA(equals(exception)),
        );
        verify(() => mockAuthServerRepository.settingAccountLinkAccountWithApple(emptyActionTokenRequest)).called(1);
      });
    });
  });
}
