...
class UserSettingHidePhoneStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = UserSettingHidePhoneTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, UserSettingHidePhoneTestConfig config) async {
  ...
  Future<void> _performAddFriend($, UserSettingHidePhoneTestConfig config) async {
  ...
  Future<void> _performAction($, UserSettingHidePhoneTestConfig config) async {
    if (config.isHidePhoneNumber) {
      await _settingPage.proceedHidePhoneNumber($);
    }
  }

  Future<void> _performExpectedSuccess($, UserSettingHidePhoneTestConfig config) async {
    await _loginPage.proceedLogout($);
    ...
    final friendNameFinder = $(config.loginByMeProfile);
    final rowFinder = find.ancestor(of: friendNameFinder, matching: find.byType(Row));
    final joinButtonFinder = $(rowFinder).$('Accept').first;
    ...
    await $.pump();
    expect($('Phone number'), findsNothing);
  ...
  Future<void> _performExpectedFail($, UserSettingHidePhoneTestConfig config) async {}

  Future<void> _performCleanup($, UserSettingHidePhoneTestConfig config) async {
...
class UserSettingHidePhoneTestConfig {
  ...
  UserSettingHidePhoneTestConfig.fromRow(Map<String, dynamic> row)
      : isRunTestCase = CommonUtil.convertStringToBoolean(row['is_run_test_case']),
        ...
        isHidePhoneNumber = CommonUtil.convertStringToBoolean(row['is_hide_phone_number']),
