...
class UserSettingQrcodeStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = UserSettingQrCodeTestConfig.fromRow(dataRow);
    if (config.isRunTestCase) {
      try {
        ...
        // 2. Add Friend & Chat Verification Flow
  ...
  Future<void> _performRegistration($, UserSettingQrCodeTestConfig config) async {
  ...
  Future<void> _performAddFriend($, UserSettingQrCodeTestConfig config) async {
  ...
  Future<void> _performAction($, UserSettingQrCodeTestConfig config) async {
    await _settingPage.proceedQrcode($, config);
  }

  Future<void> _performExpectedSuccess($, UserSettingQrCodeTestConfig config) async {
    if (config.isSaveQrcode) {
      await _loginPage.proceedLogout($);
      ...
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'QR Code');
      if (await $.platform.mobile.isPermissionDialogVisible()) {
        await $.platform.mobile.grantPermissionWhenInUse();
      }
      await CommonUtil.tapButton($, targetNameButton: AssetEntityImage);
      ...
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'Add to contact');
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: find.byType(Icon));
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'Back');
      await $.pump();
      expect($(config.loginByMeProfile), findsOne);
      await CommonUtil.tapNavigationBarWith($, 'Menu');
      await _loginPage.proceedLogout($);
    }
  }

  Future<void> _performExpectedFail($, UserSettingQrCodeTestConfig config) async {}

  Future<void> _performCleanup($, UserSettingQrCodeTestConfig config) async {
...
class UserSettingQrCodeTestConfig {
  ...
  UserSettingQrCodeTestConfig.fromRow(Map<String, dynamic> row)
      : isRunTestCase = CommonUtil.convertStringToBoolean(row['is_run_test_case']),
        ...
        loginByFriendProfile = row['login_by_friend_profile_name'] ?? '',
        loginByFriendPassword = row['login_by_friend_password'] ?? '',
        loginByMePhone = row['login_by_me_phone_number'] ?? '',
        loginByMeProfile = row['login_by_me_profile_name'] ?? '',
        ...
        isShareQrcode = CommonUtil.convertStringToBoolean(row['is_share_qr_code']),
        isSaveQrcode = CommonUtil.convertStringToBoolean(row['is_save_qr_code']),
