...
class LoginStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = LoginTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, LoginTestConfig config) async {
    for (int i = 0; i < config.regPhones.length; i++) {
      ...
      if (!config.isLoginByPhone) {
        await _loginUser($, config.phoneNumber, config.password, true, config.email);
  ...
  Future<void> _performAction($, LoginTestConfig config) async {
    await _loginUser($, config.phoneNumber, config.password, config.isLoginByPhone, config.email);
  }

  Future<void> _performExpectedSuccess($, LoginTestConfig config) async {
    expect($('Menu'), findsOneWidget);
    await CommonUtil.tapNavigationBarWith($, 'Menu');
    await _loginPage.proceedLogout($);
  }

  Future<void> _performExpectedFail($, LoginTestConfig config) async {}

  Future<void> _performCleanup($, LoginTestConfig config) async {
...
class LoginTestConfig {
  ...
  LoginTestConfig.fromRow(Map<String, dynamic> row)
