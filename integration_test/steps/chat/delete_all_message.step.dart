...
class DeleteAllMessageStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = DeleteAllMessageTestConfig.fromRow(dataRow);
    ...
    try {
      ...
      // 5. Delete Message Workflow
      if (config.isDeleteAllMessage) {
        await _loginUser($, config.loginByOwnerPhone, config.loginByOwnerPassword, true);
        await _performDeleteMessageAction($, config);
      }
  ...
  Future<void> _performRegistration($, DeleteAllMessageTestConfig config) async {
  ...
  Future<void> _performAddFriend($, DeleteAllMessageTestConfig config) async {
  ...
  Future<void> _performCreateGroup($, DeleteAllMessageTestConfig config) async {
    await _createNewGroupWorkflow.proceedCreateGroup($, config.addMemberDetail, config.groupName);
  }

  Future<void> _performJoinGroup($, LoginPage _loginPage, DeleteAllMessageTestConfig config) async {
  ...
  Future<void> _performDeleteMessageAction($, DeleteAllMessageTestConfig config) async {
    ...
    List<String> messages = config.targetMessage.split('|');
    final parentFinder = find.ancestor(of: find.text(messages[0]), matching: find.byType(MessageTypeRegularText));
    await CommonUtil.tapButtonLongPressV2($, targetNameButton: parentFinder);
    await CommonUtil.tapButton($, targetNameButton: 'Delete for all');
    await $.pump();
    //await $.pumpAndSettle();
    final unSelectedFinder = find.ancestor(of: find.text(messages[0]), matching: find.byType(GestureDetector));
    await CommonUtil.tapButton($, targetNameButton: unSelectedFinder);

    for (var message in messages.skip(0)) {
      final selectedMessageFinder = find.ancestor(of: find.text(message), matching: find.byType(GestureDetector));
      await CommonUtil.tapButton($, targetNameButton: selectedMessageFinder);
      await $.pump();
    }

    // Confirm Delete
    final deleteForAllButton = find.textContaining('Delete for all');
    await CommonUtil.tapButton($, targetNameButton: deleteForAllButton);
    await CommonUtil.tapButton($, targetNameButton: 'Delete');
    //await $.pumpAndSettle();
    await $.pump(const Duration(seconds: 1));

    // Verification
    final deletedMessageFinder = find.ancestor(
      of: find.textContaining('delete a message'),
      matching: find.byType(MessageTypeRemoveOthers),
    );
    expect(deletedMessageFinder, findsWidgets);
  ...
  Future<void> _performCleanup($, DeleteAllMessageTestConfig config) async {
...
class DeleteAllMessageTestConfig {
  ...
  DeleteAllMessageTestConfig.fromRow(Map<String, dynamic> row)
      : isDeleteAfterRun = CommonUtil.convertStringToBoolean(row['is_delete_after_run']),
        ...
        isDeleteAllMessage = CommonUtil.convertStringToBoolean(row['is_delete_all_message']),
