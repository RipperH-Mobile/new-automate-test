...
class UnsendMessageStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = UnsendMessageTestConfig.fromRow(dataRow);
    ...
    try {
      ...
      // 4. Unsend Workflow
      if (config.isUnsend) {
        await _performUnsendMessage($, config);
      }
  ...
  Future<void> _performRegistration($, UnsendMessageTestConfig config) async {
  ...
  Future<void> _performAddFriendAndChat($, UnsendMessageTestConfig config) async {
  ...
  Future<void> _performUnsendMessage($, UnsendMessageTestConfig config) async {
    await _loginUser($, config.loginByMePhone, config.loginByMePassword, config.isLoginByPhone);
    await CommonUtil.tapNavigationBarWith($, 'Chat');

    // Enter Chat Room to Perform Unsend
    ...
    // Expect system message
    parsedText = $(ParsedText).first;
    systemMessageText = $.tester.widget<ParsedText>(parsedText);
    systemMessageTextOutput = systemMessageText.text ?? "";
    expect(systemMessageTextOutput, allOf(contains(config.loginByMeProfileName), contains(config.expectedMessage)));
    await CommonUtil.tapIconBack($);
    await $.pump();
  ...
  Future<void> _performCleanup($, UnsendMessageTestConfig config) async {
...
class UnsendMessageTestConfig {
  ...
  UnsendMessageTestConfig.fromRow(Map<String, dynamic> row)
      : isDeleteAfterRun = CommonUtil.convertStringToBoolean(row['is_delete_after_run']),
        ...
        // Unsend Specific
        isUnsend = CommonUtil.convertStringToBoolean(row['is_unsend']),
