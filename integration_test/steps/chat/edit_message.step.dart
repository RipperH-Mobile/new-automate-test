...
class EditMessageStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = EditMessageTestConfig.fromRow(dataRow);
    ...
    try {
      ...
      // 3. Add Friend, Chat & Edit Message Verification
      if (config.isAddFriend) {
        await _performEditMessageAndVerify($, config);
      }
  ...
  Future<void> _performRegistration($, EditMessageTestConfig config) async {
  ...
  Future<void> _performEditMessageAndVerify($, EditMessageTestConfig config) async {
    ...
    for (final dataSet in dataSetList) {
      ...
      final inputCurrentMessage = fields[3];
      final inputNewMessage = fields[4];
      final inputPhoneNational = await CommonUtil.convertE164ToNational(inputPhoneWithCode);

      // 3.1 Add Friend & Send Initial Message
      await _chatPage.proceedAddFriend($, inputPhoneNational);
      await _chatPage.proceedChatWithMessage($, false, inputUchatId, inputCurrentMessage);
      ...
      // 3.3 Edit Message Flow
      final currentMessageFinder = find.textContaining(inputCurrentMessage);
      ...
      await $(TextField).tap();
      await $(TextField).enterText(inputNewMessage);
      ...
      await CommonUtil.tapButton($, targetNameButton: friendNameFinder);
      await $.pump();
      expect($(inputNewMessage), findsOneWidget);
      await CommonUtil.tapIconBack($);
      await $.pump();
  ...
  Future<void> _performCleanup($, EditMessageTestConfig config) async {
...
class EditMessageTestConfig {
  ...
  EditMessageTestConfig.fromRow(Map<String, dynamic> row)
