...
class GroupCountMemberStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = GroupCountMemberTestConfig.fromRow(dataRow);
    ...
    try {
      ...
      // 2. Login Main User
      await _loginUser($, config.loginByOwnerPhone, config.loginByOwnerPassword, config.isLoginByPhone);
  ...
  Future<void> _performRegistration($, GroupCountMemberTestConfig config) async {
  ...
  Future<void> _performAddFriend($, GroupCountMemberTestConfig config) async {
  ...
  Future<void> _performCreateGroup($, GroupCountMemberTestConfig config) async {
    await _createNewGroupWorkflow.proceedCreateGroup($, config.addMemberDetail, config.groupName);
  }

  Future<void> _performJoinGroup($, LoginPage _loginPage, GroupCountMemberTestConfig config) async {
    await _joinNewGroupWorkflow.proceedJoinNewGroup($, _loginPage, config);
  }

  Future<void> _performExpectedSuccess($, GroupCountMemberTestConfig config) async {
    await _loginUser($, config.loginByOwnerPhone, config.loginByOwnerPassword, config.isLoginByPhone);
    await CommonUtil.tapNavigationBarWith($, 'Contact');
    final targetGroup = find.textContaining(config.groupName);
    final Text groupText = $.tester.widget<Text>(targetGroup);
    final String fullText = groupText.data ?? "";
    final RegExp regex = RegExp(r'\((\d+)\)');
    final Match? match = regex.firstMatch(fullText);
    final String? memberCount = match?.group(1);
    expect(memberCount, config.expectedTotalMember);
    await CommonUtil.tapNavigationBarWith($, 'Menu');
    await _loginPage.proceedLogout($);
  }

  Future<void> _performExpectedFail($, GroupCountMemberTestConfig config) async {}

  Future<void> _performCleanup($, GroupCountMemberTestConfig config) async {
...
class GroupCountMemberTestConfig {
  ...
  final String expectedTotalMember;
  ...
  GroupCountMemberTestConfig.fromRow(Map<String, dynamic> row)
      : isDeleteAfterRun = CommonUtil.convertStringToBoolean(row['is_delete_after_run']),
        ...
        expectedTotalMember = row['expected_total_member'] ?? '',
