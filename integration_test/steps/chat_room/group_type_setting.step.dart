...
class GroupTypeSettingStep {
  ...
  Future<void> _performRegistration($, GroupTypeSettingTestConfig config) async {
  ...
  Future<void> _performAddFriends($, GroupTypeSettingTestConfig config) async {
  ...
  Future<void> _performCreateGroup($, GroupTypeSettingTestConfig config) async {
    await _createNewGroupWorkflow.proceedCreateGroup($, config.addMemberDetail, config.groupName);
  }

  Future<void> _performAction($, GroupTypeSettingTestConfig config) async {
    await _loginUser($, config.loginByOwnerPhone, config.loginByOwnerPassword, true);
    ...
    await CommonUtil.tapButton($, targetNameButton: groupNameFinder);
    await $.pump();
    if (!config.isPrivateGroup) {
      await CommonUtil.tapButton($, targetNameButton: 'Group type');
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'Public');
      await CommonUtil.tapButton($, targetNameButton: 'Done');
      await $.pump();
    }
    await CommonUtil.tapButton($, targetNameButton: 'Invite link');
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'Invite Link');
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'On');
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'Done');
    await $.pump();
    final parentFinder = $(DecoratedBox).$(DecoratedBox).$(Row).$(Text).last;
    final Text inviteLinkText = $.tester.widget<Text>(parentFinder);
    inviteLinkStr = inviteLinkText.data ?? "";
    ...
    await _chatPage.proceedChatWithMessage($, false, config.addMemberDetail, inviteLinkStr);
    await CommonUtil.tapNavigationBarWith($, 'Menu');
    await _loginPage.proceedLogout($);
  }

  Future<void> _performExpectedSuccess($, GroupTypeSettingTestConfig config) async {
    if (config.isPrivateGroup) {
      await _loginUser($, config.loginByMemberPhone, config.loginByMemberPassword, true);
      await CommonUtil.tapNavigationBarWith($, 'Chat');
      await CommonUtil.tapButton($, targetNameButton: config.loginByOwnerProfile);
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'UChat Messenger');
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'Request to join group');
      await $('Waiting for approval').waitUntilExists();
      ...
      final friendNameFinder = find.textContaining(config.loginByMemberProfile);
      final rowFinder = find.ancestor(of: friendNameFinder, matching: find.byType(Row));
      final nestedFinder = $(rowFinder).$('Approve').first;
      expect(nestedFinder, findsOneWidget);
      await CommonUtil.tapButton($, targetNameButton: nestedFinder);
      await CommonUtil.tapNavigationBarWith($, 'Chat');
      await CommonUtil.tapButton($, targetNameButton: config.groupName);
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: config.groupName);
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'Member');
      await $.pump();
      await $(find.textContaining(config.loginByMemberProfile)).waitUntilExists();
      await CommonUtil.tapButton($, targetNameButton: 'Back');
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'Back');
      await $.pump();
      ...
      await CommonUtil.tapButton($, targetNameButton: 'UChat Messenger');
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'Join group');
      await $.pump();
      await $(find.textContaining(config.groupName)).waitUntilExists();
      await CommonUtil.tapIconBack($);
      await $.pump();
      await CommonUtil.tapNavigationBarWith($, 'Menu');
      await _loginPage.proceedLogout($);
  ...
  Future<void> _performExpectedFail($, GroupTypeSettingTestConfig config) async {}

  Future<void> _performCleanup($, GroupTypeSettingTestConfig config) async {
...
class GroupTypeSettingTestConfig {
  ...
  GroupTypeSettingTestConfig.fromRow(Map<String, dynamic> row)
      : isDeleteAfterRun = CommonUtil.convertStringToBoolean(row['is_delete_after_run']),
        ...
        isPrivateGroup = CommonUtil.convertStringToBoolean(row['is_private_group']),
