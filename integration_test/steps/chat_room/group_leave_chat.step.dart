...
class GroupLeaveChatStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = GroupLeaveChatStepTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, GroupLeaveChatStepTestConfig config) async {
  ...
  Future<void> _performAddFriends($, GroupLeaveChatStepTestConfig config) async {
  ...
  Future<void> _performCreateGroup($, GroupLeaveChatStepTestConfig config) async {
    await _createNewGroupWorkflow.proceedCreateGroup($, config.addMemberDetail, config.groupName);
  }

  Future<void> _performJoinGroup($, LoginPage _loginPage, GroupLeaveChatStepTestConfig config) async {
    await _joinNewGroupWorkflow.proceedJoinNewGroup($, _loginPage, config);
  }

  Future<void> _performAction($, GroupLeaveChatStepTestConfig config) async {
  ...
  Future<void> _performExpectedSuccess($, GroupLeaveChatStepTestConfig config) async {
    await CommonUtil.tapNavigationBarWith($, 'Contact');
    await $.pump();
    final friendNameFinder = find.textContaining(config.groupTarget);
    expect(friendNameFinder, findsNothing);
    await CommonUtil.tapButton($, targetNameButton: AppSearchBox);
    await $(SearchBox).enterText(config.groupTarget);
  ...
  Future<void> _performExpectedFail($, GroupLeaveChatStepTestConfig config) async {}

  Future<void> _performCleanup($, GroupLeaveChatStepTestConfig config) async {
...
class GroupLeaveChatStepTestConfig {
  ...
  GroupLeaveChatStepTestConfig.fromRow(Map<String, dynamic> row)
