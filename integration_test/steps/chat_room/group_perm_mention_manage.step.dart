...
class GroupPermMentionManageStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = GroupPermMentionManageTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, GroupPermMentionManageTestConfig config) async {
  ...
  Future<void> _performAddFriend($, GroupPermMentionManageTestConfig config) async {
  ...
  Future<void> _performCreateGroup($, GroupPermMentionManageTestConfig config) async {
    await _createNewGroupWorkflow.proceedCreateGroup($, config.addMemberDetail, config.groupName);
  }

  Future<void> _performJoinGroup($, LoginPage _loginPage, GroupPermMentionManageTestConfig config) async {
    await _joinNewGroupWorkflow.proceedJoinNewGroup($, _loginPage, config);
  }

  Future<void> _performAssignAdmin($, GroupPermMentionManageTestConfig config) async {
    await _loginUser($, config.loginByOwnerPhone, config.loginByOwnerPassword, true);
  ...
  Future<void> _performAction($, GroupPermMentionManageTestConfig config) async {
    await _loginUser($, config.loginByOwnerPhone, config.loginByOwnerPassword, true);
    await CommonUtil.tapNavigationBarWith($, 'Chat');
    // Enter Group
    final groupNameFinder = find.textContaining(config.groupName);
  ...
  Future<void> _performExpectedSuccess($, GroupPermMentionManageTestConfig config) async {
    final groupNameFinder = find.textContaining(config.groupName);
    await CommonUtil.tapButton($, targetNameButton: groupNameFinder);
    await $.pump();
  ...
  Future<void> _performExpectedFail($, GroupPermMentionManageTestConfig config) async {}

  Future<void> _performCleanup($, GroupPermMentionManageTestConfig config) async {
...
class GroupPermMentionManageTestConfig {
  ...
  GroupPermMentionManageTestConfig.fromRow(Map<String, dynamic> row)
      : isRunTestCase = CommonUtil.convertStringToBoolean(row['is_run_test_case']),
        ...
        isNotSendMessages = false,
        isNotEditTheirOwnSentMessage = false,
        isNotUnsendTheirOwnSentMessages = false,
