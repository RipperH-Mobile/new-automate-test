...
class GroupPermSetDefaultStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = GroupPermSetDefaultTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, GroupPermSetDefaultTestConfig config) async {
  ...
  Future<void> _performAddFriend($, GroupPermSetDefaultTestConfig config) async {
  ...
  Future<void> _performCreateGroup($, GroupPermSetDefaultTestConfig config) async {
    await _createNewGroupWorkflow.proceedCreateGroup($, config.addMemberDetail, config.groupName);
  }

  Future<void> _performJoinGroup($, LoginPage _loginPage, GroupPermSetDefaultTestConfig config) async {
    await _joinNewGroupWorkflow.proceedJoinNewGroup($, _loginPage, config);
  }

  Future<void> _performAssignAdmin($, GroupPermSetDefaultTestConfig config) async {
    await _loginUser($, config.loginByOwnerPhone, config.loginByOwnerPassword, true);
  ...
  Future<void> _performAction($, GroupPermSetDefaultTestConfig config) async {
  ...
  Future<void> _performExpectedSuccess($, GroupPermSetDefaultTestConfig config) async {
    await $.pump();
    expect($('Set to default'), findsNothing);
    await CommonUtil.tapButton($, targetNameButton: 'Back');
    await $.pump();
    await CommonUtil.tapIconBack($);
    await $.pump();
  ...
  Future<void> _performExpectedFail($, GroupPermSetDefaultTestConfig config) async {}

  Future<void> _performCleanup($, GroupPermSetDefaultTestConfig config) async {
...
class GroupPermSetDefaultTestConfig {
  ...
  GroupPermSetDefaultTestConfig.fromRow(Map<String, dynamic> row)
      : isRunTestCase = CommonUtil.convertStringToBoolean(row['is_run_test_case']),
        ...
        isSetToDefault = CommonUtil.convertStringToBoolean(row['is_set_to_default']),
        ...
        isNotSendMedia = CommonUtil.convertStringToBoolean(row['is_not_send_media']),
        isNotDisplayMediaTools = CommonUtil.convertStringToBoolean(row['is_not_display_media_tools']),
        isNotMentionAll = CommonUtil.convertStringToBoolean(row['is_not_mention_all']),
        isNotUseEmojiReactions = CommonUtil.convertStringToBoolean(row['is_not_use_emoji_reactions']),
        isNotAddDeleteAlbumInGroup = CommonUtil.convertStringToBoolean(row['is_not_add_delete_album_in_group']),
