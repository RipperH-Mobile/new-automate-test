...
class GroupOwnershipTransferStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = GroupOwnershipTransferTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, GroupOwnershipTransferTestConfig config) async {
  ...
  Future<void> _performAddFriends($, GroupOwnershipTransferTestConfig config) async {
  ...
  Future<void> _performCreateGroup($, GroupOwnershipTransferTestConfig config) async {
    await _createNewGroupWorkflow.proceedCreateGroup($, config.addMemberDetail, config.groupName);
  }

  Future<void> _performJoinGroup($, LoginPage _loginPage, GroupOwnershipTransferTestConfig config) async {
    await _joinNewGroupWorkflow.proceedJoinNewGroup($, _loginPage, config);
  }

  Future<void> _performAction($, GroupOwnershipTransferTestConfig config) async {
    await _loginUser($, config.loginByOwnerPhone, config.loginByOwnerPassword, true);
    ...
    await CommonUtil.tapButton($, targetNameButton: groupNameFinder);
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'Ownership transfer');
    await $.pump();
    final memberNameFinder = find.textContaining(config.whoAssignToOwner);
    await CommonUtil.tapButton($, targetNameButton: memberNameFinder);
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'Confirm');
    await $.pump(const Duration(seconds: 1));
  }

  Future<void> _performExpectedSuccess($, GroupOwnershipTransferTestConfig config) async {
    final parsedText = $(MessageTypeSystemV2).$(ParsedText).first;
    final ParsedText systemMessageParsedText = $.tester.widget<ParsedText>(parsedText);
    final String systemMessageText = systemMessageParsedText.text ?? "";
    expect(systemMessageText, contains(config.expectedMessage));
    final groupNameFinder = find.textContaining(config.groupName);
    await CommonUtil.tapButton($, targetNameButton: groupNameFinder);
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'Member');
    await $.pump();
    final newOwnerNameFinder = find.ancestor(
        of: find.textContaining(config.whoAssignToOwner), matching: find.byType(DetailInviteMemberListItem));
    final targetNewOwnerNameFinder = $(newOwnerNameFinder).$(find.byType(AppText)).last;
    expect(targetNewOwnerNameFinder, findsOneWidget);
    final userTypeFinder = find.descendant(of: targetNewOwnerNameFinder, matching: find.byType(Text));
    final Text userTypeText = $.tester.widget<Text>(userTypeFinder);
    final String userTypeStr = userTypeText.data ?? "";
    expect(userTypeStr, 'Owner');
  ...
  Future<void> _performExpectedFail($, GroupOwnershipTransferTestConfig config) async {}

  Future<void> _performCleanup($, GroupOwnershipTransferTestConfig config) async {
...
class GroupOwnershipTransferTestConfig {
  ...
  final bool isOwnershipTransfer;
  final String whoAssignToOwner;
  ...
  GroupOwnershipTransferTestConfig.fromRow(Map<String, dynamic> row)
      : isDeleteAfterRun = CommonUtil.convertStringToBoolean(row['is_delete_after_run']),
        ...
        isOwnershipTransfer = CommonUtil.convertStringToBoolean(row['is_ownership_transfer']),
        whoAssignToOwner = row['who_assign_to_owner'] ?? '',
