...
class EditChatListStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = EditChatListTestConfig.fromRow(dataRow);
    ...
    try {
      ...
      await CommonUtil.tapIconHambergerBar($);
      await $.pump();
      await CommonUtil.tapButton($, targetNameButton: 'Edit chat list');
  ...
  Future<void> _performRegistration($, EditChatListTestConfig config) async {
  ...
  Future<void> _performAddFriendAndChat($, EditChatListTestConfig config) async {
  ...
  Future<void> _performSelectReadAllAction($, EditChatListTestConfig config) async {
    await CommonUtil.tapButton($, targetNameButton: 'Read All');
    await $.pump();
    final animatedFlipCounterFinder = find.byType(AnimatedFlipCounter);
    await $.pump();
    expect(animatedFlipCounterFinder, findsNothing);
    await CommonUtil.tapNavigationBarWith($, 'Menu');
    await _loginPage.proceedLogout($);
  }

  Future<void> _performSelectHideAction($, EditChatListTestConfig config) async {
    List<String> chats = config.chatTarget.split('|');
    for (var chat in chats) {
      var friendNameFinder = find.textContaining(chat);
      final rowFinder = find.ancestor(of: friendNameFinder, matching: find.byType(Row));
      await CommonUtil.tapButton($, targetNameButton: rowFinder);
    }
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: find.textContaining('Hide'));
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'Confirm');
    await $.pump();
    for (var chat in chats) {
      var friendNameFinder = find.textContaining(chat);
      expect(friendNameFinder, findsNothing);
    }
    await CommonUtil.tapNavigationBarWith($, 'Menu');
    await _loginPage.proceedLogout($);
  }

  Future<void> _performSelectDeleteAction($, EditChatListTestConfig config) async {
    List<String> chats = config.chatTarget.split('|');
    for (var chat in chats) {
      var friendNameFinder = find.textContaining(chat);
      final rowFinder = find.ancestor(of: friendNameFinder, matching: find.byType(Row));
    ...
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: find.textContaining('Delete'));
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'Confirm');
    await $.pump();
    for (var chat in chats) {
  ...
  Future<void> _performExpectedSuccess($, EditChatListTestConfig config) async {
    if (config.isSelectReadAll) {
      await _performSelectReadAllAction($, config);
    }
    if (config.isSelectHide) {
      await _performSelectHideAction($, config);
    }
    if (config.isSelectDelete) {
      await _performSelectDeleteAction($, config);
    }
  }

  Future<void> _performExpectedFail($, EditChatListTestConfig config) async {}

  Future<void> _performCleanup($, EditChatListTestConfig config) async {
...
class EditChatListTestConfig {
  ...
  final bool isSelectReadAll;
  final bool isSelectHide;
  ...
  EditChatListTestConfig.fromRow(Map<String, dynamic> row)
      : isDeleteAfterRun = CommonUtil.convertStringToBoolean(row['is_delete_after_run']),
        ...
        isSelectReadAll = CommonUtil.convertStringToBoolean(row['is_select_read_all']),
        isSelectHide = CommonUtil.convertStringToBoolean(row['is_select_hide']),
        isSelectDelete = CommonUtil.convertStringToBoolean(row['is_select_delete']),
