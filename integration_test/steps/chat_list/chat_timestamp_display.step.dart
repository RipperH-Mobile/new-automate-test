...
class ChatTimestampDisplayStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = ChatTimestampDisplayTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, ChatTimestampDisplayTestConfig config) async {
  ...
  Future<void> _performAddFriendAndChat($, ChatTimestampDisplayTestConfig config) async {
  ...
  Future<void> _performExpectedSuccess($, ChatTimestampDisplayTestConfig config) async {
    ...
    final nestedFinder = $(rowFinder).$(find.byType(AppText));
    expect(nestedFinder, findsOneWidget);
    final textFinder = find.descendant(of: nestedFinder, matching: find.byType(Text));
    final Text textWidget = $.tester.widget<Text>(textFinder);
    final String timeString = textWidget.data ?? "";
    var outputFormat = DateFormat(config.expectedTimeFormat);
    expect(isValidDateFormat(outputFormat, timeString), isTrue);
    await CommonUtil.tapNavigationBarWith($, 'Menu');
    await _loginPage.proceedLogout($);
  }

  bool isValidDateFormat(DateFormat format, String value) {
    try {
      format.parseStrict(value);
      return true;
    } catch (e, stackTrace) {
      debugPrint('---isValidDateFormat---$e\n$stackTrace');
      return false;
    }
  }

  Future<void> _performExpectedFail($, ChatTimestampDisplayTestConfig config) async {}

  Future<void> _performCleanup($, ChatTimestampDisplayTestConfig config) async {
...
class ChatTimestampDisplayTestConfig {
  ...
  ChatTimestampDisplayTestConfig.fromRow(Map<String, dynamic> row)
      : isDeleteAfterRun = CommonUtil.convertStringToBoolean(row['is_delete_after_run']),
        ...
        expectedTimeFormat = row['expected_time_format'] ?? '',
