...
class MarkAsReadStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = MarkAsReadTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, MarkAsReadTestConfig config) async {
  ...
  Future<void> _performAddFriendAndChat($, MarkAsReadTestConfig config) async {
  ...
  Future<void> _performExpectedSuccess($, MarkAsReadTestConfig config) async {
    ...
    await $.pump();
    await CommonUtil.tapButton($, targetNameButton: 'Mark as read');
    final countNumberFinder = $(rowFinder).$(animatedFlipCounterFinder);
    expect(countNumberFinder, findsNothing);
    await CommonUtil.tapNavigationBarWith($, 'Menu');
    await _loginPage.proceedLogout($);
  }

  Future<void> _performExpectedFail($, MarkAsReadTestConfig config) async {}

  Future<void> _performCleanup($, MarkAsReadTestConfig config) async {
...
class MarkAsReadTestConfig {
  ...
  MarkAsReadTestConfig.fromRow(Map<String, dynamic> row)
