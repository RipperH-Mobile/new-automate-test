...
class LastMessagePreviewDisplayStep {
  ...
  Future<List<Map<String, dynamic>>> proceedTest($, dataRow, resultsData) async {
    ...
    final config = LastMessagePreviewDisplayTestConfig.fromRow(dataRow);
  ...
  Future<void> _performRegistration($, LastMessagePreviewDisplayTestConfig config) async {
  ...
  Future<void> _performAddFriendAndChat($, LastMessagePreviewDisplayTestConfig config) async {
  ...
  Future<void> _performExpectedSuccess($, LastMessagePreviewDisplayTestConfig config) async {
    ...
    final mentionWrapperFinder = find.byType(MentionTextParse);
    final nestedFinder = $(rowFinder).$(mentionWrapperFinder);
    expect(nestedFinder, findsOneWidget);
    final richTextFinder = find.descendant(of: nestedFinder, matching: find.byType(RichText));
    ...
    expect(actualText, config.expectedMessage);
    await CommonUtil.tapNavigationBarWith($, 'Menu');
    await _loginPage.proceedLogout($);
  }

  Future<void> _performExpectedFail($, LastMessagePreviewDisplayTestConfig config) async {}

  Future<void> _performCleanup($, LastMessagePreviewDisplayTestConfig config) async {
...
class LastMessagePreviewDisplayTestConfig {
  ...
  LastMessagePreviewDisplayTestConfig.fromRow(Map<String, dynamic> row)
