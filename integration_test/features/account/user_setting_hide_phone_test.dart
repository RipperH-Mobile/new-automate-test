...
import '../../steps/account/user_setting_hide_phone.step.dart';

void main() {
  late List<Map<String, dynamic>> testData;
  late List<Map<String, dynamic>> resultsData = [];
  setUpAll(() async {
    final dataPath = 'assets/temp_test_data/user_setting_hide_phone.csv';
    testData = await TestDataUtil.loadData(dataPath);
    await CommonUtil.createMockLoggerService();
  });
  patrolTest('User setting hide phone scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await UserSettingHidePhoneStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename = 'feature_account_user_setting_hide_phone_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
