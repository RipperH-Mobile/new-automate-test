  ...
  patrolTest('User setting pin lock scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await UserSettingPinLockStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename = 'feature_account_user_setting_pin_lock_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
