...
void main() {
  ...
  setUpAll(() async {
    final dataPath = 'assets/temp_test_data/pin_message.csv';
    testData = await TestDataUtil.loadData(dataPath);
    await CommonUtil.createMockLoggerService();
  });
  patrolTest('Pin message scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await PinMessageStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename = 'feature_chat_pin_message_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
