  ...
  patrolTest('Delete message successful', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await DeleteMessageStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  late List<Map<String, dynamic>> resultsData = [];
  patrolTest('Delete message scenario', ($) async {
    try {
      final dataPath = 'assets/temp_test_data/delete_message.csv';
      late List<Map<String, dynamic>> testData;
      testData = await TestDataUtil.loadData(dataPath);
  ...
  tearDown(() async {
    ...
    const String outputFilename = 'feature_chat_delete_message_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
