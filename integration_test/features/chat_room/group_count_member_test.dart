  ...
  patrolTest('Group count member scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await GroupCountMemberStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename = 'feature_chat_room_group_count_member_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
