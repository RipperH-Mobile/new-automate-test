...
import '../../steps/chat_room/group_delete_history_chat.step.dart';

void main() {
  late List<Map<String, dynamic>> testData;
  late List<Map<String, dynamic>> resultsData = [];
  setUpAll(() async {
    final dataPath = 'assets/temp_test_data/group_delete_history_chat.csv';
    testData = await TestDataUtil.loadData(dataPath);
    await CommonUtil.createMockLoggerService();
  });
  patrolTest('Group delete history chat scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await GroupDeleteHistoryChatStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename =
        'feature_chat_room_group_delete_history_chat_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
