...
import '../../steps/chat_room/group_ownership_transfer.step.dart';

void main() {
  late List<Map<String, dynamic>> testData;
  late List<Map<String, dynamic>> resultsData = [];
  setUpAll(() async {
    final dataPath = 'assets/temp_test_data/group_ownership_transfer.csv';
    testData = await TestDataUtil.loadData(dataPath);
    await CommonUtil.createMockLoggerService();
  });
  patrolTest('Group ownership transfer scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await GroupOwnershipTransferStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename =
        'feature_chat_room_group_ownership_transfer_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
