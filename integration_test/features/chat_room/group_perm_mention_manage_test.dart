...
import '../../steps/chat_room/group_perm_mention_manage.step.dart';

void main() {
  late List<Map<String, dynamic>> testData;
  late List<Map<String, dynamic>> resultsData = [];
  setUpAll(() async {
    final dataPath = 'assets/temp_test_data/group_perm_mention_manage.csv';
    testData = await TestDataUtil.loadData(dataPath);
    await CommonUtil.createMockLoggerService();
  });
  patrolTest('Group permission set to default scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await GroupPermMentionManageStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename =
        'feature_chat_room_group_perm_mention_manage_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
