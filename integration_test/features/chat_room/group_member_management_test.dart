...
import '../../steps/chat_room/group_member_management.step.dart';
...
void main() {
  ...
  setUpAll(() async {
    final dataPath = 'assets/temp_test_data/group_member_management.csv';
    testData = await TestDataUtil.loadData(dataPath);
    await CommonUtil.createMockLoggerService();
  });
  patrolTest('Group member management scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await GroupMemberManagementStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename =
        'feature_chat_room_group_member_management_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
