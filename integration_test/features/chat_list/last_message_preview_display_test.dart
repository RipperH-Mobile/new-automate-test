...
import '../../steps/chat_list/last_message_preview_display.step.dart';

void main() {
  late List<Map<String, dynamic>> testData;
  late List<Map<String, dynamic>> resultsData = [];
  setUpAll(() async {
    final dataPath = 'assets/temp_test_data/last_message_preview_display.csv';
    testData = await TestDataUtil.loadData(dataPath);
    await CommonUtil.createMockLoggerService();
  });
  patrolTest('Last message preview scenario', ($) async {
    try {
      ...
      for (final dataRow in testData) {
        resultsData = await LastMessagePreviewDisplayStep().proceedTest($, dataRow, resultsData);
...
void runTests() {
  ...
  tearDown(() async {
    ...
    const String outputFilename =
        'feature_chat_list_last_message_preview_display_test_${deviceTarget}_${envTarget}_batch.csv';
    Directory directory;
    if (Platform.isIOS) {
