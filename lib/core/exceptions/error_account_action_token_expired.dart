...
class ErrorAccountActionTokenExpired implements ApiException {
  @override
  ...
  ErrorAccountActionTokenExpired({
  ...
  factory ErrorAccountActionTokenExpired.fromMap(Map<String, dynamic> data) {
    DataApiException? dataApiException;
    if (data['data'] != null) {
      dataApiException = DataApiException.fromMap(data['data']);
    }

    return ErrorAccountActionTokenExpired(
  ...
  @override
  String toString() {
    return 'ErrorAccountActionTokenExpired: $code ($type): $name -> $message';
  }
