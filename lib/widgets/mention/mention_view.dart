...
class FlutterMentionsState extends State<FlutterMentions> {
  ...
  Map<String, Annotation> mapToAnnotation() {
    ...
    for (var element in widget.mentions) {
      ...
      final dataSorted = element.data.sorted(
        (a, b) {
          return (b['display']?.length ?? 0) - (a['display']?.length ?? 0);
        },
      );

      for (var e in dataSorted) {
  ...
  void addMention(Map<String, dynamic> value, [Mention? list]) {
    ...
    final list0 = widget.mentions.firstWhere(
  ...
  void inputListeners() {
    ...
    if (widget.onMarkupChanged != null) {
      widget.onMarkupChanged!(controller!.markupText[0]);
    }
      ...
      portalFollower: ValueListenableBuilder(
        ...
        builder: (BuildContext context, bool show, Widget? child) {
          if (show && !widget.hideSuggestionList) {
            ...
            final searchMembers = list.data.where((element) {
              final ele = element['display'].toLowerCase();
              // return ele == str ? false : ele.contains(str);
              return ele.contains(str);
            });

            final List<Map<String, dynamic>> suggestionMembers;

            if (str != '') {
              suggestionMembers = searchMembers.sorted((a, b) {
                final String nameA = (a['display'] ?? '').toLowerCase();
                final String nameB = (b['display'] ?? '').toLowerCase();

                final foundIndexA = nameA.indexOf(str);
                final foundIndexB = nameB.indexOf(str);

                if (foundIndexA < foundIndexB) {
                  return foundIndexB;
                } else if (foundIndexA > foundIndexB) {
                  return foundIndexA;
                }
              ...
              suggestionMembers = searchMembers.toList();
            }
