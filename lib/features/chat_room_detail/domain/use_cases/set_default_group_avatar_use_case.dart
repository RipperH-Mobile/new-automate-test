import 'package:get_it/get_it.dart';
import 'package:uchat/features/chat_room_detail/data/models/requests/set_default_group_avatar_request.dart';
import 'package:uchat/features/chat_room_detail/domain/repositories/chat_room_detail_server_repository.dart';
import 'package:uchat/use_cases/use_case.dart';

class SetDefaultGroupAvatarUseCase extends SimpleUseCase<void, SetDefaultGroupAvatarRequest> {
  ChatRoomDetailServerRepository get chatRoomDetailServerRepository {
    return GetIt.I<ChatRoomDetailServerRepository>();
  }

  @override
  Future<void> call(SetDefaultGroupAvatarRequest params) {
    return chatRoomDetailServerRepository.setDefaultGroupAvatar(params);
  }
}
