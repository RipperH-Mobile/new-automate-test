import 'package:get/get.dart';
import 'package:uchat/features/chat_room_detail/presentation/controllers/chat_room_detail_album_image_list_controller.dart';

class ChatRoomDetailAlbumImageListBinding implements Bindings {
  @override
  void dependencies() {
    final tag = Get.parameters['id'] ?? 'NEW_ROOM';

    Get.put<ChatRoomDetailAlbumImageListController>(
      ChatRoomDetailAlbumImageListController(tag: tag),
      tag: tag,
    );
  }
}
