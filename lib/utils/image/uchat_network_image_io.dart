...
import 'uchat_network_image_provider.dart' as image_provider;

final _log = useLogger();

class UChatNetworkImageProvider extends ImageProvider<image_provider.UChatNetworkImageProvider>
    with ExtendedImageProvider<image_provider.UChatNetworkImageProvider>
    implements image_provider.UChatNetworkImageProvider {
  /// Creates an object that fetches the image at the given URL.
  ///
  /// The arguments must not be null.
  UChatNetworkImageProvider(
    this.url, {
    this.scale = 1.0,
    this.headers,
    this.cache = false,
  ...
  /// Network response when get file
  HttpClientResponse? _response;

  @override
  ImageStreamCompleter loadImage(
    image_provider.UChatNetworkImageProvider key,
    ...
    return MultiFrameImageStreamCompleter(
      codec: _loadAsync(
        key as UChatNetworkImageProvider,
        chunkEvents,
        decode,
      ),
      scale: key.scale,
      ...
      informationCollector: () {
        return <DiagnosticsNode>[
          ...
          DiagnosticsProperty<image_provider.UChatNetworkImageProvider>(
            'Image key',
            key,
          ),
        ];
  ...
  @override
  Future<UChatNetworkImageProvider> obtainKey(ImageConfiguration configuration) {
    return SynchronousFuture<UChatNetworkImageProvider>(this);
  }

  Future<ui.Codec> _loadAsync(
    UChatNetworkImageProvider key,
    ...
    if (cache) {
      try {
        ...
        if (printError) {
          _log.e('Cannot _loadAsync.', e, stackTrace);
        }
      }
    }
  ...
  Future<Uint8List?> _loadNetwork(
    ...
    try {
      ...
      if (bytes.lengthInBytes == 0) {
        return Future<Uint8List>.error(StateError(
          'NetworkImage is an empty file: $resolved',
        ));
      }

      return bytes;
    } on OperationCanceledError catch (e, stackTrace) {
      if (printError) {
        _log.e('User cancel request $url.', e, stackTrace);
      }
  ...
  bool operator ==(dynamic other) {
    ...
    return other is UChatNetworkImageProvider &&
        url == other.url &&
        scale == other.scale &&
  ...
  // We set `autoUncompressed` to false to ensure that we can trust the value of
  // the `Content-Length` HTTP header. We automatically uncompressed the content
  ...
  @override
  int? get responseCode => _response?.statusCode;
}
