...
import 'package:uchat/utils/timeago/timeago.dart' as timeago_lib;
...
extension Date on DateTime {
  ...
  DateTime addMinutes(int amount, [bool ignoreDaylightSavings = false]) => ignoreDaylightSavings
      ? DateTime(year, month, day, hour, minute + amount, second, millisecond, microsecond)
  ...
  DateTime addSeconds(int amount, [bool ignoreDaylightSavings = false]) => ignoreDaylightSavings
      ? DateTime(year, month, day, hour, minute, second + amount, millisecond, microsecond)
  ...
  int get getBuddhistYear => year + 543;
  ...
  DateTime get startOfWeek => weekday == DateTime.sunday ? startOfDay : subDays(weekday).startOfDay;
  ...
  String format(String pattern, [String? locale]) {
    initializeDateFormatting();
    locale ??= Get.locale?.toLanguageTag() ?? 'en_US';
    final adjustPattern = _adjustTimePattern(pattern);

    if (locale.startsWith('th')) {
      return formatToThai(adjustPattern).to12HourFormat();
    }
    
    return DateFormat(adjustPattern, locale).format(this).to12HourFormat();
  }

  String formatToThai(String pattern) {
    return DateFormat(pattern, 'th_TH').formatBuddhist(this);
  }
  ...
  DateTime removeSecond() {
    return copyWith(second: 0, millisecond: 0);
  }

  String _adjustTimePattern(String pattern) {
    final context = Get.context;

    if (context == null) return pattern;

    final use24HourFormat = SystemDateTimeFormat.of(context).timePattern?.startsWith('H') ?? true;

    if (use24HourFormat) return pattern;

    return pattern.replaceAllMapped(
      RegExp(r'(HH|kk):mm(:ss\.S)?'),
      (match) => 'h:mm${match.group(2) ?? ''} a',
    );
  }
}
