...
class SettingBlockFriendsController extends GetxController {
  ...
  void onInit() async {
    ...
    _contactUpdateSubscription = eventBus.on<ContactUpdateEvent>().listen((event) {
      if (event.contact.isBlocked == false) {
  ...
  Future<void> getContactsToState() async {
    try {
      final entityList = await GetIt.I<GetBlockedContactUseCase>().call(NoParams());
  ...
  void handleUnblock(ContactCollection contact) {
    UChatNewDialog.showMultipleActionsDialog(
      ...
      actions: [
        UChatNewDialogAction(
          ...
          onPressed: () => onConfirmUnBlock(contact),
        ),
        UChatNewDialogAction(
          title: AppText.button2Bold(
            context: Get.context!,
  ...
  Future<void> onConfirmUnBlock(ContactCollection contact) async {
    try {
      UChatLoading.show(status: 'Updating...'.tr);

      await GetIt.I<UnblockContactUseCase>().call(
        UnblockContactParams(contactIds: [contact.id!]),
      );
    ...
    UChatNewDialog.showDialog(
      ...
      onConfirm: () async {
        try {
          ...
          final result = await GetIt.I<RemoveFriendUseCase>().call(
            RemoveFriendRequest(friendAccountId: contact.id!),
          );
          if (result) {
